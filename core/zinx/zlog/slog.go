...
func SetLogFile2(fileDir string, fileName string) {
	fs, _ := os.OpenFile(fmt.Sprintf(`%v/%v`, fileDir, fileName), os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0666)
...
func Info2(v string, arg ...any) {
	slog.Info(fmt.Sprintf(`%v%v`, v, command.SliceJoin(arg, " ")))
}
...
func Warn2(v string, arg ...any) {
	slog.Warn(fmt.Sprintf(`%v%v`, v, command.SliceJoin(arg, " ")))
}
...
func Error2(v string, arg ...any) {
	slog.Error(fmt.Sprintf(`%v%v`, v, command.SliceJoin(arg, " ")))
}
